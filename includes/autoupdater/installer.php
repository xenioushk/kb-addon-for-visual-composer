...
{
  // bkbm_kavc_installation
  $apiURL = bkbmKavcApiUrl();
  $site_url = get_site_url();
  $product_id = BKB_VC_ADDON_CC_ID; // change the id
  $ip = $_SERVER['REMOTE_ADDR'];
  $ver = BKB_VC_ADDON_CURRENT_VERSION;
  ...
  if (is_array($output) && !is_wp_error($output) && wp_remote_retrieve_response_code($output) === 200) {
    ...
    if (isset($output_decode['status']) && $output_decode['status'] != 0) {
      ...
      update_option(BKB_VC_ADDON_INSTALLATION_TAG, '1'); // change the tag
