...
class DependencyManager {
	...
	private static function set_dependency_constants() {
		define( 'KAFWPB_MIN_BKBM_VERSION', '1.5.7' );
	...
	public static function check_minimum_version_requirement_status() {
		...
		return ( version_compare( BKBM_CURRENT_PLUGIN_VERSION, KAFWPB_MIN_BKBM_VERSION, '>=' ) );
	}
	...
	public static function notice_min_version_main_plugin() {
		...
		$message = sprintf(
            ...
            esc_html__( 'The %2$s requires %1$s %4$s or higher. You are using %3$s', 'bkb_vc' ),
            self::$bkbm_url,
            self::$addon_title,
            BKBM_CURRENT_PLUGIN_VERSION,
            KAFWPB_MIN_BKBM_VERSION
        );
